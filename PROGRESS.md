...
| Build the AI agent simulator | In progress |
...
Data processing (Phase 2) is complete. Phase 3 (the AI simulator) has
started, with the model-communication layer done.

- Next: the simulator itself — the personas (role archetypes, never named
  real Enron people), the scenarios, and the prompt design that generates
  simulated email responses using S_shots as the real stimuli.
- ⚠️ **One thing needs your input before any money is spent:** the config
  file lists placeholder identifiers for the OpenAI models (the second AI
  family, used so the judge is never the same model as the writer). They're
  marked TODO and are probably out of date. They need confirming against
  OpenAI's current model list before the first paid run.
