...
def test_scenario_grid_is_fully_crossed() -> None:
    ...
    assert len(scenarios) == len(TASK_TYPES) * len(DIRECTIONS) * len(STAKES) * len(TONES)
    assert len({s.scenario_id for s in scenarios}) == len(scenarios)


def test_every_tone_appears() -> None:
    tones = {s.tone for s in build_scenarios()}
    assert tones == set(TONES)
